...
def main():
    ...
    try:
        ...
        for index, instance in enumerate(minst_instances):
            if instance.network.stem == "mnist-net_256x4":
                result_summary["TOTAL_RUNS"] = result_summary["TOTAL_RUNS"] + 1
                break

            print(
                f"Verifying instance {index+1} of {len(minst_instances)}: {instance.network.stem} {instance.property.stem}"
            )
            ...
            if verification_result.value.result == "ERR":
                result_summary["ERR_FILES"].append(
                    {
                        ...
                        "ERROR": verification_result.value.stdout.rstrip().splitlines()[
                            :3
                        ]
                        + ["..."]
                        + verification_result.value.stdout.rstrip().splitlines()[
                            -3:
                        ],
                ...
                print(
                    f"ERROR: {verification_result.value.stdout.rstrip().splitlines()[0]}"
                )
    ...
    with open(f"benchmark_results_abcrown_cifar_conf.json", "w") as f:
